    ...
    <div class="row dashboard-nowrap">
        ...
        <div class="m-4 p-4 width-medium">
            <div class="m-4 p-4 border-dashed">
                <c:out value="Id: ${tweet.id}"/><br>
                <c:out value="User: ${tweet.user.username}"/><br>
                <c:out value="Date: ${tweet.formattedTime}"/><br>
                <c:out value="Text: ${tweet.text}"/><br>
                <c:out value="Comments:"/><br>
                <c:forEach items="${tweet.comments}" var="comment">
                    <c:out value="Date: ${comment.formattedTime}, User: ${comment.user.username}, Text: ${comment.text}"/><br>
                </c:forEach>
                <a href="/tweet/edit/${tweet.id}">Edit</a>
                <a href="/tweet/delete/${tweet.id}" class="confirm">Delete</a>
            </div>
        </div>
    </div>
</section>
